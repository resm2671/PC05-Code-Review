...
# Fix #7 Moved squiggles forward and turning after color choice statements, so that the very first draw is consistent with the color scheme desired in comments.
    squiggles.forward(random.randint(4,19)) #move forward some random amount
# Fix #3 Removed one space from next line to fix IndentationError. Indent did not match.
    squiggles.left(random.randint(-170,170))
